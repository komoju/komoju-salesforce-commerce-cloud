...
var Countries = require('*/cartridge/scripts/util/Countries.ds');
 ...
 * If neither address is available, it prepopulates with the default address of the authenticated customer.
 @param {Object} cart contains the cart
 */
 ...
 * is used.
 @param {Object} cart contains the current basket
 */
...
function returnToForm(cart, params) {
    ...
    Transaction.wrap(function () {
        ...
        Object.keys(paymentInstruments).forEach(function (key) {
            currentBasket.removePaymentInstrument(paymentInstruments[key]);
        });
    });
 ...
 * @param {Object} cart contains the current basket
 * @return {Object} return the error object
...
function billing() {
    app.getForm('billing').handleAction({
        ...
        save: function () {
            var currentBasket = BasketMgr.getCurrentBasket();
            var method = request.httpParameterMap.options.value;
            var selectedPaymentMethod = PaymentMgr.getPaymentMethod(app.getForm('billing').object.paymentMethods.selectedPaymentMethodID.value);
            Transaction.wrap(function () {
